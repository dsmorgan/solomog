...
| **5 — mesh (stretch)** | 2× vsphere clusters + `gateway`-topology Istio mesh (existing `mesh-eastwest.sh`; **no** networking.sh analog needed — east-west gateways get real MetalLB IPs) | `istioctl multicluster check` green across two hl clusters. **ACCEPTED LIVE (2026-08-11, s6+s7, community ambient)** — functional: a client pod on s6 got HTTP 200 from a service whose only endpoints live on s7 (ztunnel's sole route = SplitHorizonWorkload via s7's east-west gateway); formal: `istioctl remote-clusters` shows both istiods `synced` to their peer via the remote secrets (upstream istioctl has no `multicluster check` — that's Solo's; `remote-clusters` is the upstream discovery check, plus `ztunnel-config service/workload` for the data-path view). `solomog istio:ambient:multi-gateway CLUSTERS="s6 s7" EDITION=community` — same task as vind; mesh.sh uses registered clusters as-is (must pre-exist; no mixing vind+external; flat refused for externals) and skips all host routing. **Community/upstream gaps found & fixed en route** (latent on vind too): ztunnel chart needed `multiCluster.clusterName` + `global.network` (default "Kubernetes" is rejected by istiod); istiod needs `AMBIENT_ENABLE_MULTI_NETWORK` + `AMBIENT_ENABLE_BAGGAGE` (gated on ISTIO_MULTI_NETWORK, exported by mesh.sh for gateway topology); upstream's east-west class is **istio-east-west** (vs Solo's istio-eastwest — mesh-eastwest.sh auto-detects); upstream remote-peer Gateways need trust-domain cluster.local + BOTH listeners (15008+15012); istio-system ns needs the `topology.istio.io/network` label; and upstream discovery = **remote secrets** (istio-reader kubeconfigs, replicated with plain kubectl — the ambient e/w gateway can't expose istiod). k3s also needs `global.platform=k3s` (ISTIO_PLATFORM, exported by mesh.sh for vsphere targets). |
...
| 13 | VIP stability | **REMOVED (2026-08-10, David's call)** — name-sticky VIP pinning (phase 6a: `lb-*` allocator roles, `metallb.io/loadBalancerIPs` pin, 60s settle-wait, `PURGE_LB` keep/purge semantics) existed to make a *manually maintained* DNS record permanent. Once decision 10 automated the record (expose upserts it every run and tracks VIP changes), pinning served nothing the wildcard cert + upsert don't already cover. VIPs are now plain MetalLB auto-assign, and `vsphere:delete` deletes the cluster's DNS=real records via the OPNsense API (matched on the descr expose stamps; best-effort, no flags). **Amended same day after live multi-cluster testing (3 clusters, all VIPs = .131)**: independent per-cluster MetalLBs handed the same full pool pick the SAME address (the assignment hashes the service identity, identical across clusters) and ARP-battle — the removed allocator had also been the cross-cluster pool partitioner. Fix: `vsphere:create` reserves a contiguous per-cluster **slice** of `VSPHERE_LB_POOL` (`vsphere_alloc_lb_slice`, `VSPHERE_LB_SLICE_SIZE` default 4 → 29-IP pool = 7 clusters) via the ippool file, and the cluster's IPAddressPool CR carries only that slice; MetalLB auto-assigns within it. Slices release on delete; stopped clusters keep theirs. ⚠ The pool-hygiene lesson survives the allocator's ping-guard: `VSPHERE_LB_POOL` must contain only free IPs — MetalLB does no liveness check (ds2's 10G leg ARP-battled the VIP). |
